WITH t1 AS (
    ...
    SELECT  
        CASE 
            WHEN score_interno <= 150 THEN 'Ate 150'
            WHEN score_interno > 150 and score_interno <= 300 THEN 'Entre 151 e 300'
            WHEN score_interno > 300 and score_interno <= 450 THEN 'Entre 301 e 450'
            WHEN score_interno > 450 and score_interno <= 600 THEN 'Entre 451 e 600'
            WHEN score_interno > 600 and score_interno <= 750 THEN 'Entre 601 e 750'
            ELSE 'Acima de 750'
        END AS faixa_score_interno,
    ...
    GROUP BY faixa_score_interno

    ORDER BY percentual_inadimplentes DESC
)

SELECT faixa_score_interno, total_valor_solicitado FROM t1