WITH t1 AS (
    ...
    SELECT  
        CASE 
            WHEN comprometimento_renda <= .10 THEN 'De 0% a 10%'
            WHEN comprometimento_renda > .10 and comprometimento_renda <= .20 THEN 'De 10% a 20%'
            WHEN comprometimento_renda > .20 and comprometimento_renda <= .30 THEN 'De 20% a 30%'
            WHEN comprometimento_renda > .30 and comprometimento_renda <= .40 THEN 'De 30% a 40%'
            ELSE 'Mais de 40%'
    ...
    GROUP BY faixa_comprometimento_renda

    ORDER BY faixa_comprometimento_renda
)

SELECT faixa_comprometimento_renda, total_valor_solicitado FROM t1